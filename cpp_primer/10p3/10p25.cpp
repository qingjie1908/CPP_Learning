...
bool check_size(std::string &s1, std::vector<std::string>::size_type check_sz)
{
    return s1.size() < check_sz;
}
...
void biggies(std::vector<std::string> &words, std::vector<std::string>::size_type sz)
{
    ...
    //using bind instead of lambda
    //this binded newcallable take only 1 paramenter, which is the element in words, then pass it chech_size func first paramter
    //check_size() second parameter has argument sz
    auto iter = std::stable_partition(words.begin(), words.end(), std::bind(check_size, std::placeholders::_1, sz));

    //how many element has size larger than sz?
    //auto count = words.end() - iter;

    //using count_if algorithm instead of auto count = words.end() - iter;
    //a predicate that it applies to each element in the given input range
    // returns a count of how often the predicate is true.
    auto count = std::count_if(words.begin(), words.end(), [](const std::string &s){return s.size() >= 5;});
    std::cout << "There are " << count << " element in this string vector whose size is larger than " << sz << "." << std::endl; //3
