...
protected:
   ...
   NotQuery(const NotQuery& orig) = default;
   ...
   //if a class has a copy constructor but does not define a move constructor In this case, the compiler will not synthesize the move constructor. so it will use copy control even we intend to move them by call move
   //so explicity defined move contro;
   NotQuery(NotQuery&& orig) = default;
